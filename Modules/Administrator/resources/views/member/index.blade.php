        ...
        <div class="x_panel">
            ...
            <div class="x_content">
                ...
                <div class="form-group">
                    ...
                    <button type="button" name="tloEnable" onclick="CrudMember('create','*')" class="btn btn-sm btn-outline-secondary"><i class="fa fa-plus"></i> Create</button>
                    @endif
...
@include('administrator::member.partials.CrudMember')
<script>
    $(document).ready(function() {

        $("#jqGridMain").jqGrid({
            url: "{{ url('administrator/jsonMember') }}",
            datatype: "json",
            mtype: "GET",
            postData: {
                id: "1",
            ...
            colModel: [{
                ...
                formatter: function(cellvalue, options, rowObject) {
                    var status = rowObject.status_member == 1 ? 'Active' : 'Inactive';
                    var badge = rowObject.status_member == 1 ? 'badge-success' : 'badge-danger';
        ...
        function actionBarangFormatter(cellvalue, options, rowObject) {
            ...
            if (CrudMenuPermission($MenuUrl, $user_id, 'edit')) { ?>
                btn += `<button data-id="${btnid}" onclick="CrudMember('update','${btnid}')"  class="btn btn-sm text-white btn-option badge-success"><i class="fa fa-pencil"></i></button>`;
            <?php } else { ?>
            ...
            <?php if (CrudMenuPermission($MenuUrl, $user_id, 'delete')) { ?>
                btn += `<button  data-id="${btnid}" onclick="CrudMember('delete','${btnid}')" class="btn btn-sm text-white btn-option badge-danger"><i class="fa fa-remove"></i></button>`;
            <?php } else { ?>
    ...
    function CrudMember(action, idx) {

        if (action == "create") {
            document.getElementById("formCrudMember").reset();
            $("#formCrudMember .form-control").prop("disabled", false);
            $(".btn-title").html('<i class="fa fa-save"></i> Create')
            $("#titleModal").html('Create Member')
            $('#modalCrudMember').modal('show');
            $('#CrudMemberError').html("");
            $("#CrudMemberAction").val('create');
            $("#CrudMemberAlertDelete").html('');
            ...
            $("#CrudMemberAlertDelete").html(errMsg)
        }
    }

    function details(idx) {
        $.ajax({
            url: '{{ url("administrator/jsonDetailMember") }}',
            type: 'POST',
            method: 'post',
            data: {
                id: idx,
